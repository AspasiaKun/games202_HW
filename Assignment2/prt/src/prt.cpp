...
namespace ProjEnv
{
                                                    ...
                                                    const int &channel)
    {
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l = 0; l < SHOrder; l++) {
                        for (int m = -l; m <= l; m++) {
                            auto SHCoeff = Le * sh::EvalSH(l,m,dir.cast<double>().normalized()) * dw;
                            SHCoeffiecents[l*(l+1)+m] += SHCoeff;
                        }
                    }
                }
            }
        ...
        for (int i =0;i<SHNum;i++) {
            std::cout<<SHCoeffiecents[i]<<std::endl;
        }
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                if (m_Type == Type::Unshadowed)
                {
                    ...
                    float h = wi.normalized().dot(n.normalized());
                    return h > 0.0? h:0.0;
                }
                else
                {
